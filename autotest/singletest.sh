...
bash engine/${engine}/entrypoint_outer.sh --model-owner "$model_owner" --model-name "$model_name" --port "$port" --tp-size "$tp_size"

# wait until server is ready
check_server http://127.0.0.1:${port}/v1/models

# call benchmark/${benchmark}.sh
bash benchmark/${benchmark}/entrypoint_outer.sh --concurrency "$concurrency" --model-owner "$model_owner"  --model-name "$model_name" --interval "$interval" --input-tokens "$input_tokens" --output-tokens "$output_tokens" --port "$port" --tp-size "$tp_size"

# exit engine
bash engine/${engine}/exit.sh

log "Completed test: benchmark=$benchmark, concurrency=$concurrency, engine=$engine, model: $model_owner/$model_name, interval=$interval, input_tokens=$input_tokens, output_tokens=$output_tokens, port=$port, tp_size=$tp_size"
